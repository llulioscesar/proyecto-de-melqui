        ...
        <div id="pdf" ref="pdf" class="bg-white q-py-xs q-px-md" style="max-width:720px;width:100%;margin:auto;">
          ...
          <div class="q-pa-md ritem q-mb-lg" style="border:1px solid black">
            ...
            <tr v-for="(item, i) in info" :key="i">
              <td class="text-left" style="border-bottom:1px dashed black;">{{item.mes}}</td>
              <td class="text-left" style="border-bottom:1px dashed black;">{{$currency.format(item.total)}}</td>
            </tr>
          </table>
...
export default {
  ...
  mounted(){
    this.info = JSON.parse(JSON.stringify(this.$q.localStorage.get.item('infoVenta')))
